  ...
  <div class="row ml-3">
    <?php 
	  if(isset($_POST['view-reservations'])){
		  ...
		  $sql  = "SELECT A.RES_NUM,A.CID,A.START_DT,A.END_DT,B.RT_ID,C.HID,C.TYPE,D.USERNAME,E.NAME FROM `reservation` A,`rooms` B,`roomTypes` C,`customers` D,`hotels` E WHERE A.RID=B.RID AND A.CID=D.CID AND C.RT_ID=B.RT_ID AND E.HID=C.HID AND A.CID=" . $_POST['userID'] . ";";
		...
		$fetch = $mysqli->query($sql) or die("Reservation info not available");
		  ...
		  $sql  = "SELECT A.RES_NUM,A.CID,A.START_DT,A.END_DT,B.RT_ID,C.HID,C.TYPE,D.USERNAME,E.NAME FROM `reservation` A,`rooms` B,`roomTypes` C,`customers` D,`hotels` E WHERE A.RID=B.RID AND A.CID=D.CID AND C.RT_ID=B.RT_ID AND E.HID=C.HID ;";
